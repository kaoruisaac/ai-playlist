...
export type Track = {
  id: string; title: string; artist: string; playlistRole: string; introduction: string; playbackSource: PlaybackSource;
};
export type Playlist = { id: string; title: string; description: string; tracks: Track[]; createdAt: string; updatedAt: string };
export type ChatMessage = { id: string; role: "user" | "agent" | "system"; kind: "conversation" | "playlist-ready" | "track-introduction" | "error"; content: string; createdAt: string; trackId?: string };
...
export type PlaylistSession = { schemaVersion: 3; id: string; createdAt: string; updatedAt: string; originalRequest: string; interpretedVibe?: string; messages: ChatMessage[]; playlist?: Playlist; playback: PlaybackState; agentSettings: AgentSettings; introducedTrackIds: string[] };
