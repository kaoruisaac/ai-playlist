...
export async function connectPedelec(
  ...
  const tools = [
    ...
    defineTool({ name: "append_tracks", description: "Append researched tracks in order. Each track has only title, artist, playlistRole, and a reliable 11-character YouTube videoId.", argsSchema: appendTracksArgsSchema, handler: async (args: unknown, ctx) => {
